...
class SimpleMLP:
    def __init__(self, config: NetworkConfig) -> None:
        ...
        self.params: dict[str, np.ndarray] = {
            "W1": (rng.standard_normal((config.input_size, config.hidden_size1)) * np.sqrt(2.0 / config.input_size)).astype(
                np.float32
            ),
            "b1": np.zeros(config.hidden_size1, dtype=np.float32),
            "W2": (rng.standard_normal((config.hidden_size1, config.hidden_size2)) * np.sqrt(2.0 / config.hidden_size1)).astype(
                np.float32
            ),
            "b2": np.zeros(config.hidden_size2, dtype=np.float32),
            "W3": (rng.standard_normal((config.hidden_size2, config.output_size)) * np.sqrt(2.0 / config.hidden_size2)).astype(
                np.float32
            ),
    ...
    def from_state(cls, state: dict[str, object]) -> "SimpleMLP":
        ...
        config = NetworkConfig(
            ...
            hidden_size1=int(config_dict["hidden_size1"]),
            hidden_size2=int(config_dict["hidden_size2"]),
